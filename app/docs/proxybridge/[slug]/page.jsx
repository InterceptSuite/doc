...
export async function generateStaticParams() {
  return getAllDocSlugs('proxybridge').map((slug) => ({ slug }))
}

export async function generateMetadata({ params }) {
  try {
    const doc = getDocBySlug(params.slug, 'proxybridge')
    return {
      title: doc.title,
...
function findAdjacentDocs(slug) {
  const allItems = proxyBridgeNavigation.flatMap((g) => g.items)
...
function findCategory(slug) {
  return proxyBridgeNavigation.find((g) => g.items.some((item) => item.slug === slug))
}

export default function ProxyBridgeDocPage({ params }) {
  let doc
  try {
    doc = getDocBySlug(params.slug, 'proxybridge')
  } catch {
    notFound()
  }

  const allDocs = getAllDocs('proxybridge')
  const allBlogs = getAllBlogs()
  const searchData = [
    ...getAllDocs('interceptsuite').map((d) => ({ ...d, type: 'docs' })),
        ...
        <div className="mx-auto w-full max-w-[1200px] px-6">
          <div className="flex gap-6 items-start">
            <div className="hidden lg:block sticky top-24 self-start h-[calc(100vh-6rem)] overflow-y-auto flex-shrink-0">
              <DocSidebar navigation={proxyBridgeNavigation} basePath="/docs/proxybridge" />
            </div>
            ...
            <main className="flex-1 min-w-0 rounded-2xl border border-[#1A1A1A] bg-card p-8 md:p-10 xl:p-12">
              <div className="mb-6 flex items-center gap-2 text-sm text-content-dim">
                ...
                <span>/</span>
                <Link href="/docs" className="hover:text-content transition-colors">Docs</Link>
                <span>/</span>
                <Link href="/docs/proxybridge" className="hover:text-content transition-colors">ProxyBridge</Link>
                {category && (
                  <>
                    <span>/</span>
              ...
              <div className="mt-12 pt-8 border-t border-[#1A1A1A] flex items-center justify-between gap-4">
                {prev ? (
                  <Link
                    href={`/docs/proxybridge/${prev.slug}`}
                ...
                {next ? (
                  <Link
                    href={`/docs/proxybridge/${next.slug}`}
              ...
              <div className="mt-8 pt-6 border-t border-[#1A1A1A]">
                <a
                  href={`https://github.com/InterceptSuite/ProxyBridge/tree/main/docs/${params.slug}.md`}
                  target="_blank"
